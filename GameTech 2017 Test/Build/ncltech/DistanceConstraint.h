...
public:
	...
	virtual void ApplyImpulse() override
	{
		...
		//Compute the velocity of object's A and B at the point of contact
		...
		float abnVel = Vector3::Dot(v0 - v1, abn); //Relative velocity in constraint direction



												   //Compute the 'mass' of the constraint
												   // e.g. How difficult it is to move the two objects in the direction of the constraint
		...
		if (constraintMass > 0.0f)
		{
			...
			// - Experiment by commenting this out and see how it affects the constraints over time and when you manually move the objects apart.
			// - The key is to find a nice value that is small enough not to cause objects to explode but also
			//   enough to make sure all constraints /will/ be satisfied. This value (0.1) will change based on your physics objects, timestep etc
			//   and also how many constraints you are chaining together.
			float b = 0.0f;
			//-Optional-	
			...
			// In order to satisfy the distance constraint we need to apply forces to ensure
			// the relative velocity (abnVel) in the direction of the constraint is zero. So
			// we take inverse of the current rel velocity and multiply it by how hard it will be to
			// move the objects. 
			// Note: We also add in any extra energy to the system here, e.g. baumgarte (and later elasticity)
