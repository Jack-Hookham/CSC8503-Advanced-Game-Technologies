...
bool CollisionDetectionSAT::AreColliding(CollisionData* out_coldata)
{
	...
	//  provided above. (This is only needed when dealing with 3D collision shapes)
	//  As mentioned in the tutorial, this should be the edge vector's not normals we test against, however for a cuboid 
	//  example this is the same as testing the normals as each normal /will/ match a given edge elsewhere on the object.
	//  For more complicated geometry, this will have to be changed to actual edge vectors instead of normals.
	...
	{
		//If the collision axis does NOT intersect then return immediately as we know that atleast
		// in one direction/axis the two objects do not intersect
...
bool CollisionDetectionSAT::CheckCollisionAxis(const Vector3& axis, CollisionData& out_coldata)
{
	...
	if (A <= C && B >= C)
	{
		...
		out_coldata._penetration = C - B; //Smallest overlap distance is between B->C
	...
	if (C <= A && D >= A)
	{
		out_coldata._normal = -axis; //Invert axis here so we can do all our resolution phase as Object 1 -> Object 2
		out_coldata._penetration = A - D; //Smallest overlap distance is between D->A
 ...
 /* TUTORIAL 5 CODE */
	...
	else if (polygon1.size() == 1)
	{
		out_manifold->AddContact(
			...
			polygon1.front() + bestColData._normal * bestColData._penetration,
			bestColData._normal,
	...
	{
		out_manifold->AddContact(
			polygon2.front() - bestColData._normal * bestColData._penetration,
	...
	else
	{
		...
		//First we need to know if have to flip the incident and reference faces around for clipping
		...
		for (const Vector3& point : polygon2)
		{
			...
			//If we flipped incident and reference planes, we will need to flip it back before
			// sending it to the manifold. e.g. turn it from talking about object2->object1
			// into object1->object2
			if (flipped)
			{
