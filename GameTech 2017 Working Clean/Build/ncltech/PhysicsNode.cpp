...
void PhysicsNode::IntegrateForVelocity(float dt)
{
	/* TUTORIAL 2 CODE */
	//Apply Gravity
	//	Technically gravity here is calculated by formula: ( gravity / invMass * invMass * dt )
	...
	//  - In it's present form this can be seen as a rough approximation of air resistance, albeit (wrongly?) making the assumption that all objects
...
void PhysicsNode::IntegrateForPosition(float dt)
{
	...
	//Update Orientation
	// - This is slightly different calculation due to the wierdness of quaternions. It does the same thing as position update
	//  (with a slight error) but from i've seen, is generally the best way to update orientation.
	orientation = orientation + Quaternion(angVelocity * dt * 0.5f, 0.0f) * orientation;

	//invInertia = invInertia * (Quaternion(angVelocity * dt * 0.5f, 0.0f) * orientation).ToMatrix3();
	//As the above formulation has slight approximation error, we need to normalize our orientation here to stop them accumulating
	// over time.
